...
page: pda
---
A high-efficiency PDA workflow interface for warehouse staff to scan and verify parcels.
...
**Page Structure:**
1.  **Header:** Shows current batch number (`activeBatch.batch_number`) and role (`sender`/`transit`/`receiver`).
2.  **Main Interaction Area:**
    *   Giant input field for barcode entry (`inputValue`, `inputRef`).
    *   Manual verification button (`handleManualVerify`).
3.  **Result Feedback (List):**
    *   Displays recent `batchParcels`.
    *   Active scanned item highlighted.
    *   Status badges (`passed`, `anomaly`) using the color palette.
4.  **Thumb Zone (Footer):**
    *   "Complete" button (Primary Action).
    *   "Skip" button (Secondary/Danger Action).
    *   Must be large tap targets for gloved usage.
