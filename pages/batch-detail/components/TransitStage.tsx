...
export const TransitStage: React.FC<TransitStageProps> = ({ batch, shipments, inspections }) => {
    ...
    const totalTransitWeight = Object.values(parsedInspections).reduce((sum, i) => sum + (i.transit_weight || 0), 0);
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1">
                            ...
                            <div className="flex items-end gap-2">
                                <span className="text-2xl font-bold font-mono text-slate-900 dark:text-white">{totalTransitWeight.toLocaleString(undefined, { minimumFractionDigits: 1 })}</span>
                                {diff !== 0 && (
                        ...
                        <div className="space-y-1 pl-4 border-l border-slate-100 dark:border-slate-700">
                            ...
                            <div className="flex items-end gap-2 text-slate-900 dark:text-white">
                                <span className="text-2xl font-bold font-mono">{totalSenderWeight.toLocaleString(undefined, { minimumFractionDigits: 1 })}</span>
                            </div>
                            <div className="w-full bg-slate-100 dark:bg-slate-700 h-1.5 rounded-full mt-2 overflow-hidden">
            ...
            <div className="px-4 space-y-3 pb-24">
                {shipments.map((s) => {
                    ...
                    return (
                        <div
                            key={s.id}
                            className={`bg-white dark:bg-[#1c2433] p-4 rounded-lg border shadow-sm relative overflow-hidden transition-all ${(hasDiff || dimDiff)
                        ...
                        >
                            ...
                            <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-xs">
                                ...
                                <div className="flex flex-col">
                                    ...
                                    <span className="font-mono text-slate-600 dark:text-slate-300">{s.weight?.toFixed(2)} kg</span>
                                </div>
                                <div className={`flex flex-col ${hasDiff ? 'bg-amber-50 dark:bg-amber-900/10 -mx-2 px-2 rounded' : ''}`}>
                                    <span className={`text-[10px] ${hasDiff ? 'text-amber-600 dark:text-amber-500' : 'text-slate-400'}`}>
                                        重量 {hasDiff ? `(差异 ${weightDiff > 0 ? '+' : ''}${weightDiff.toFixed(2)})` : ''}
                                    </span>
                                    <span className={`font-mono ${hasDiff ? 'text-amber-600 dark:text-amber-400 font-bold' : 'text-slate-900 dark:text-white font-semibold'}`}>
